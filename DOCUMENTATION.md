...
1. **`:root`** — Dark theme defaults (always applied).
2. **`[data-theme="light"]`** — Overrides specific variables for light theme.
3. **`var(--variable)`** — Used throughout stylesheet for consistent theming.
...
1. Add a new object to the `ROOMS_DATA` array in `script.js`:
...
2. If adding a new **type category**, add a corresponding filter button in `index.html`:
...
1. Create an API service module (e.g., `api.js`):
...
2. Replace `localStorage` calls in `script.js` with async API calls.
3. Add user authentication (JWT/sessions) for personalized booking management.
4. Add a server-side route layer (Node.js/Express, Python/Flask, etc.).
